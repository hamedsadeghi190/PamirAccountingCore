...
namespace PamirAccounting.Forms.Checks
{
    public partial class PaymentCheckList : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == dataGridView1.Columns["btnRowDelete"].Index && e.RowIndex >= 0)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    {
                        var adminRole = unitOfWork.UserInRoleServices.FindFirstOrDefault(x => x.Role.Code == (int)Permission.Admin && x.UserId == CurrentUser.UserID);
                        var roleId = unitOfWork.UserInRoleServices.FindFirstOrDefault(x => x.Role.Code == (int)Permission.DeleteDepositDocument && x.UserId == CurrentUser.UserID);
                        if (roleId == null && adminRole == null)
                        {
                            MessageBox.Show(Messages.PermissionMsg);
                            return;
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            var data = new UnitOfWork().ChequeServices.GetAllPayment();
            var basedata = new reportbaseDAta() { Date = PersianDate };
            var report = StiReport.CreateNewReport();
            report.Load(AppSetting.ReportPath + "PaymentCheckList.mrt");
        ...
        private void PaymentCheckList_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.F7)
            {
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    if (roleId != null || adminRole != null)
                    {
                        ...
                        var frm = new DetailsPaymentCheckFrm(dataList.ElementAt(rowIndex).Id);
                        frm.ShowDialog();
                        LoadData();
                    }
                }
