        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == dataGridView1.Columns["btnRowDelete"].Index && e.RowIndex >= 0)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    {
                        var adminRole = unitOfWork.UserInRoleServices.FindFirstOrDefault(x => x.Role.Code == (int)Permission.Admin && x.UserId == CurrentUser.UserID);
                        var roleId = unitOfWork.UserInRoleServices.FindFirstOrDefault(x => x.Role.Code == (int)Permission.DeleteBargashtDaryafti && x.UserId == CurrentUser.UserID);
                        if (roleId == null && adminRole == null)
                        {
                            MessageBox.Show(Messages.PermissionMsg);
                            return;
                        ...
                        if (roleId != null || adminRole != null)
                        {
                            ...
                            log.Date = DateTime.Parse(DateTime.Now.ToShortDateString());
                            log.Time = DateTime.Now.TimeOfDay;
                            log.UserId = CurrentUser.UserID;
                            log.UserName = CurrentUser.UserName;
                            log.DocumentId = cheque.DocumentId;
                            log.Description = $"حذف چک برگشتی به شماره {cheque.ChequeNumber} به مبلغ {cheque.Amount}، شماره سند {cheque.DocumentId} ";
